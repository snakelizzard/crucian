...
public:
  ...
  inline SparseBinaryMatrix(size_type nrows, size_type ncols)
      : ncols_(ncols), ind_(nrows), buffer_(ncols) {}
  ...
  /**
   * Fills this matrix with random rows that all have the same number of
   * non-zeros.
   ...
   */
  inline void compact() {
    if (capacity() == nNonZeros() && buffer_.size() == buffer_.capacity())
      return;
  ...
  /**
   * Returns a binary vector of size nRows, which contains true if the
   * corresponding
   * row is zero, false otherwise. Also returns number of zero rows.
   */
  template <typename OutputIterator>
  inline size_type zeroRowsIndicator(OutputIterator it,
                                     OutputIterator it_end) const {
    { NTA_ASSERT(static_cast<size_type> (it_end - it) == nRows()); }
   ...
   */
  template <typename OutputIterator>
  inline size_type nonZeroRowsIndicator(OutputIterator it,
                                        OutputIterator it_end) const {
    { NTA_ASSERT(static_cast<size_type> (it_end - it) == nRows()); }
  ...
  inline void nNonZerosPerRow(OutputIterator begin, OutputIterator end) const {
    { // Pre-conditions
      NTA_ASSERT(static_cast<size_type> (end - begin) == nRows())
  ...
  inline void nNonZerosPerCol(OutputIterator begin, OutputIterator end) const {
    { // Pre-conditions
      NTA_ASSERT(static_cast<size_type> (end - begin) == nCols())
    ...
    std::fill(begin, end, static_cast<size_type> (0));
  ...
  inline size_type nNonZerosInRowRange(size_type row, size_type col_begin,
    ...
    return static_cast<size_type> (c2 - c1);
  }
  ...
  inline void setAllNonZeros(size_type nrows, nz_index_type ncols,
    ...
    { // Pre-conditions
      ...
      if (nz_i_end != nz_i && clean) {
        ...
        for (; ii != nz_i_end; iip = ii, jjp = jj, ++ii, ++jj) {
          NTA_ASSERT(*iip < *ii || *jjp < *jj)
              << where << "Repeated or out-of-order non-zero indices: "
              << "(" << *iip << ", " << *jjp << ") and (" << *ii << ", " << *jj
              << ")";
        }
      }
      ...
      for (; iii != nz_i_end; ++iii, ++jjj) {
        NTA_ASSERT(*iii < nrows) << where << "Invalid row index: " << *iii
                                 << " - Should be < number of rows: " << nrows;
        NTA_ASSERT(*jjj < ncols) << where << "Invalid col index: " << *jjj
  ...
  inline void appendDenseRow(InputIterator begin, InputIterator end) {
    { // Pre-conditions
      NTA_ASSERT(static_cast<size_type> (end - begin) == nCols())
          ...
          << "Invalid vector size: " << static_cast<size_type> (end - begin)
  ...
  inline void appendSparseCol(InputIterator ind, InputIterator ind_end) {
    { // Pre-conditions
      NTA_ASSERT(static_cast<size_type> (ind_end - ind) <= nRows())
          << "SparseBinaryMatrix::appendSparseCol: "
          << "Invalid vector size: " << static_cast<size_type> (ind_end - ind)
  ...
  template <typename InputIterator>
  inline void replaceSparseRow(size_type row, InputIterator begin,
                               InputIterator end) {
    { // Pre-conditions
      NTA_ASSERT(/*0 <= row &&*/ row < nRows())
  ...
  template <typename InputIterator>
  inline size_type findRowSparse(InputIterator begin, InputIterator end) const {
    { // Pre-conditions
  ...
  template <typename InputIterator>
  inline size_type findRowDense(InputIterator begin, InputIterator end) const {
    { // Pre-conditions
      NTA_ASSERT(static_cast<size_type> (end - begin) == nCols())
  ...
  minHammingDistance(InputIterator begin, InputIterator end) const {
    ...
    for (size_type row = 0; row != nRows(); ++row) {
      ...
      d += static_cast<size_type> (end - it);
   ...
   */
  template <typename InputIterator>
  inline size_type firstRowCloserThan(InputIterator begin, InputIterator end,
  ...
  template <typename InputIterator>
  inline size_type firstRowCloserThan_dense(InputIterator begin,
                                            InputIterator end,
                                            size_type distance) const {
    size_type nnzr = 0;
    ...
    return firstRowCloserThan(buffer_.begin(), buffer_.begin() + nnzr,
                              distance);
  }
  ...
  inline void setRangeToOne(size_type row, size_type begin, size_type end) {
    ...
    for (nz_index_type j = begin; j != end; ++j)
      buffer_[j] = static_cast<size_type> (1);
  ...
  inline void logicalOr(const SparseBinaryMatrix &o) {
    { // Pre-conditions
      NTA_ASSERT(o.nRows() == nRows())
          ...
          << "Mismatch in number of rows: " << nRows() << " and: " << o.nRows();

      NTA_ASSERT(o.nCols() == nCols())
          << "SparseBinaryMatrix::logicalOr: "
          << "Mismatch in number of cols: " << nCols() << " and: " << o.nCols();
    } // End pre-conditions
    ...
    for (size_type r = 0; r != nrows; ++r) {
      for (nz_index_type c = 0; c != ncols; ++c) {
        if (filled[r * ncols + c] + filled2[c * nrows + r] > 2)
          filled[r * ncols + c] = static_cast<size_type> (1);
        else
          filled[r * ncols + c] = static_cast<size_type> (0);
      }
    }
      ...
      for (size_type r = 0; r != nrows; ++r)
        for (nz_index_type c = 0; c != ncols; ++c)
          if (buffer[r * ncols + c] == 1 &&
              (c == 0 || c == ncols - 1 || buffer[r * ncols + c - 1] == 0 ||
               buffer[r * ncols + c + 1] == 0))
            new_edges[r * ncols + c] = 1;

      for (nz_index_type c = 0; c != ncols; ++c)
        for (size_type r = 0; r != nrows; ++r)
          if (buffer[r * ncols + c] == 1 &&
              (r == 0 || r == nrows - 1 || buffer[(r - 1) * ncols + c] == 0 ||
               buffer[(r + 1) * ncols + c] == 0))
            new_edges[r * ncols + c] = 1;
  ...
  inline void overlap(InputIterator x, InputIterator x_end, OutputIterator y,
                      OutputIterator y_end) const {
    {
      NTA_ASSERT(static_cast<size_type> (x_end - x) == nCols());
      NTA_ASSERT(static_cast<size_type> (y_end - y) == nRows());
    }
  ...
  */
  ...
  inline void fromBinary(std::istream &inStream) {
    ...
    { // Pre-conditions
      NTA_CHECK(inStream.good()) << where << "Bad stream";
    } // End pre-conditions
  ...
  inline void toBinary(std::ostream &outStream) const {
    { // Pre-conditions
      NTA_CHECK(outStream.good()) << "SparseBinaryMatrix::toBinary: Bad stream";
    } // End pre-conditions

    outStream << getVersion(true) << " " << nRows() << " " << nCols() << " ";
  ...
  inline void fromSparseVector(size_type nrows, size_type ncols,
    ...
    { // Pre-conditions
      ...
      */
      ...
      NTA_ASSERT(static_cast<size_type> (end - begin) <= nrows * ncols)
          << "SparseBinaryMatrix::fromSparseVector: "
          << "Invalid number of non-zero indices: " << static_cast<size_type> (end - begin)
          << "when nrows is: " << nrows << "ncols is: " << ncols;

      for (InputIterator it = begin; it != end; ++it)
        NTA_ASSERT(*it <= nrows * ncols)
            << "SparseBinaryMatrix::fromSparseVector: "
            << "Invalid index: " << *it << " in sparse vector - Should be < "
            << nrows * ncols;

      for (size_type i = 1; i < static_cast<size_type> (end - begin); ++i)
  ...
  template <typename OutputIterator>
  inline size_type toSparseVector(OutputIterator begin, OutputIterator end,
                                  size_type offset = 0) const {
    { // Pre-conditions
      NTA_ASSERT(nNonZeros() <= static_cast<size_type> (end - begin))
    ...
    return static_cast<size_type> (begin - begin1);
  }

  template <typename InputIterator>
  inline void rowFromDense(size_type row, InputIterator begin,
                           InputIterator end) {
    { // Pre-conditions
      NTA_ASSERT(/*0 <= row &&*/ row < nRows())
      ...
      NTA_ASSERT(static_cast<size_type> (end - begin) == nCols())
          << "SparseBinaryMatrix::rowFromDense: "
          << "Invalid vector size " << static_cast<size_type> (end - begin) << " vs. "
          << nCols();
    } // End pre-conditions
    ...
    for (InputIterator it = begin; it != end; ++it)
      if (!nearlyZero(*it))
        ind_[row].push_back(static_cast<size_type> (it - begin));
  }

  template <typename OutputIterator>
  inline void rowToDense(size_type row, OutputIterator begin,
    ...
    for (it = ind_[row].begin(); it != ind_[row].end(); ++it)
      *(begin + *it) = static_cast<value_type> (1);
  }

  template <typename OutputIterator>
  inline void getRow(size_type row, OutputIterator begin,
                     OutputIterator end) const {
    rowToDense(row, begin, end);
  }

  template <typename OutputIterator>
  inline void getColToDense(size_type col, OutputIterator dense,
                            OutputIterator dense_end) const {
    {
      NTA_ASSERT(col < nCols());
      NTA_ASSERT(static_cast<size_type> (dense_end - dense) == nRows());
    }
  ...
  inline void setSlice(size_type dst_first_row, size_type dst_first_col,
    ...
    for (size_type row = 0; row != static_cast<size_type> (src.nRows()); ++row) {
      for (size_type col = 0; col != static_cast<size_type> (src.nCols()); ++col) {
  ...
  template <typename InputIterator>
  inline void fromDense(size_type nrows, size_type ncols, InputIterator begin,
                        InputIterator end) {
    { // Pre-conditions
      /*
      NTA_ASSERT(0 <= nrows)
      ...
      */
      ...
      NTA_ASSERT(nrows * ncols <= static_cast<size_type> (end - begin))
          << "SparseBinaryMatrix::fromDense: "
          << "Invalid number of rows and columns: " << nrows
          << " and: " << ncols
          << " when storage has size: " << static_cast<size_type> (end - begin);
    } // End pre-conditions
  ...
  inline void toDense(OutputIterator begin, OutputIterator end) const {
    { // Pre-conditions
      NTA_ASSERT(nRows() * nCols() <= static_cast<size_type> (end - begin))
          ...
          << "Not enough memory: " << static_cast<size_type> (end - begin)
    ...
    for (size_type row = 0; row != nRows(); ++row) {
      ...
      for (nz_index_type k = 0; k != ind_[row].size(); ++k)
        *(p + ind_[row][k]) = static_cast<size_type> (1);
    }
  }

  inline void print(std::ostream &outStream) const {
    { // Pre-conditions
      NTA_CHECK(outStream.good()) << "SparseBinaryMatrix::print: Bad stream";
    } // End pre-conditions

    Row buffer(nCols());

    for (size_type row = 0; row != nRows(); ++row) {
      std::fill(buffer.begin(), buffer.end(), static_cast<size_type> (0));
      for (nz_index_type k = 0; k != ind_[row].size(); ++k)
        buffer[ind_[row][k]] = static_cast<size_type> (1);
      for (nz_index_type col = 0; col != nCols(); ++col)
  ...
  inline void rightVecSumAtNZ(InputIterator x, InputIterator x_end,
    ...
    { // Pre-conditions
      NTA_ASSERT(static_cast<size_type> (x_end - x) >= nCols())
          ...
          << " Invalid input vector size: " << static_cast<size_type> (x_end - x)
          << " - Should >= number of colums: " << nCols();

      NTA_ASSERT(static_cast<size_type> (y_end - y) >= nRows())
          << "SparseBinaryMatrix::rightVecSumAtNZ: "
          << "Invalid output vector size: " << static_cast<size_type> (y_end - y)
  ...
  inline void rightVecSumAtNZ(InputIterator x, InputIterator x_end,
    ...
    for (size_type i = 0; i != nRows(); ++i) {
      ...
      for (size_type j = 0; j != nnzr; ++j)
        s += static_cast<value_type> (x[row[j]]);
      if (s != 0)
  ...
  inline void rightVecSumAtNZ(const Buffer<T> &x,
                              SparseVector<T1, T2> &y) const {
    typedef T2 value_type;
    size_type k = 0;
  ...
  /**
   * Matrix vector multiplication on the left side, optimized because we know
   * that
   * the values of all the non-zeros are 1: there is no need to do
   * multiplications.
   */
  template <typename InputIterator, typename OutputIterator>
  inline void leftVecSumAtNZ(InputIterator x, InputIterator x_end,
                             OutputIterator y, OutputIterator y_end) const {
    { // Pre-conditions
      NTA_ASSERT(static_cast<size_type> (x_end - x) >= nRows())
          << "SparseBinaryMatrix::leftVecSumAtNZ: "
          << " Invalid input vector size: " << static_cast<size_type> (x_end - x)
          << " - Should be  >= number of rows: " << nRows();

      NTA_ASSERT(static_cast<size_type> (y_end - y) >= nCols())
          << "SparseBinaryMatrix::leftVecSumAtNZ: "
          << "Invalid output vector size: " << static_cast<size_type> (y_end - y)
          << " - Should be >= number of columns: " << nCols();
    } // End pre-conditions
    ...
    std::fill(y, y_end, static_cast<value_type> (0.0));
  ...
  template <typename InputIterator, typename OutputIterator>
  inline void vecMaxProd(InputIterator x, InputIterator x_end, OutputIterator y,
                         OutputIterator y_end) const {
    { // Pre-conditions
      NTA_ASSERT(static_cast<size_type> (x_end - x) >= nCols())
  ...
  /**
   * Finds the arg max of the values of x corresponding to non-zeros, for each
   * row.
   * The operation is:
   *
   *  for row in [0,nrows):
  ...
  inline void rightVecArgMaxAtNZ(InputIterator x, OutputIterator y) const {
    ...
    for (size_type row = 0; row != nRows(); ++row) {
      ...
      *y++ = static_cast<value_type> (max_ind);
    }
  }
  ...
  inline void leftVecMaxAtNZ(InputIterator x, OutputIterator y) const {
    ...
    std::fill(y, y + nCols(),
              static_cast<value_type> (-std::numeric_limits<value_type>::max()));
    ...
    for (size_type i = 0; i != nCols(); ++i)
      if (y[i] == static_cast<value_type> (-std::numeric_limits<value_type>::max()))
        y[i] = 0;
  }
...
private:
  ...
  inline void sparse_row_invariants_(InputIterator begin, InputIterator end,
    ...
    NTA_ASSERT(static_cast<size_type> (end - begin) <= nCols())
        ...
        << "Invalid sparse vector size: " << static_cast<size_type> (end - begin)
  ...
  inline void nCols(size_type ncols) {
    ...
    ncols_ = static_cast<nz_index_type> (ncols);
  }

  template <typename OutputIterator>
  inline void fillLine_(size_type row, OutputIterator out,
                        OutputIterator out_end, bool reverse = false) {
    { // Pre-conditions
      NTA_ASSERT(/*0 <= row &&*/ row < nRows())
          << "SparseBinaryMatrix::fillLine_: "
          << "Invalid row index: " << row;

      NTA_ASSERT(nCols() <= static_cast<size_type> (out_end - out))
          << "SparseBinaryMatrix::fillLine_: "
          << "Insufficient memory for result";
    } // End pre-conditions

    if (reverse) {
      int i = static_cast<int> (ind_[row].size() - 1);
      while (i - 1 >= 0) {
        if (ind_[row][i] - 1 == ind_[row][i - 1])
          --i;
        else {
          int begin = static_cast<int> (ind_[row][i] - 1);
          int end = static_cast<int> (ind_[row][i - 1]);
      ...
      size_type i = 0;
      while (static_cast<size_type> (i + 1) < ind_[row].size()) {
        if (ind_[row][i] + 1 == ind_[row][i + 1])
          ++i;
        else {
...
inline std::istream &operator>>(std::istream &in_stream,
  ...
  if (io_control.sparse_io == BINARY)
