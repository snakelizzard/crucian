 ...
 * depends on it.
 */
template <typename T> struct Buffer : public std::vector<T>
{
    ...
    inline Buffer(size_type _s = 0) : std::vector<T>(_s), nnz(0) {}

    inline void clear() { nnz = 0; }
    ...
    inline void push_back(const T &x) { (*this)[nnz++] = x; }
...
// a membership board (set), on darwin86, unsigned short is fastest on
// 8/11/2010. The clear() method provides a kind of incremental reset. Assumes
// the elements that are set are sparse, that there aren't many compared to the
// size of the board.
template <typename I, typename T> struct DirectAccess
{
    typedef I size_type;
    typedef T value_type;
    ...
    inline void set(size_type w, const T &v = T(1))
    {
        ...
        if (board[w] == T())
        { // that if doesn't doest much at all (verified)
            who.push_back(w);
            // assert(std::set<size_type>(who.begin(),who.end()).size() ==
            // who.size());
        }
    ...
    inline const T &operator[](size_type w) const { return board[w]; }
...
template <typename I, typename T> struct Indicator;

template <typename I> struct Indicator<I, unsigned short>
{
    typedef I size_type;
    ...
    template <typename It> inline void set_from_sparse(It begin, It end)
    {
...
struct SparseVector : public Buffer<std::pair<T1, T2>>
{
    ...
    inline SparseVector(size_type s = 0) : Buffer<std::pair<T1, T2>>(s) {}
};
