...
class LoggingException : public Exception
{
  public:
    ...
    ~LoggingException() override;

    const char *getMessage() const override
    {
        // Make sure we use a persistent string. Otherwise the pointer may
        // become invalid.
        // If the underlying stringstream object hasn't changed, don't
        // regenerate lmessage_. This is important because if we catch this
        // exception a second call to exception.what() will trash the buffer
        // returned by a first call to exception.what()
        if (!lmessageValid_)
        {
            lmessage_ = ss_.str();
            lmessageValid_ = true;
