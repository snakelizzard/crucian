...
public:
  ...
  CState &operator=(const CState &o) {
    NTA_ASSERT(_nCells == o._nCells); // _nCells should be static, since it is
...
public:
  ...
  CStateIndexed &operator=(CStateIndexed &o) {
    NTA_ASSERT(_nCells == o._nCells); // _nCells should be static, since it is
                                      // the same size for all CStates
    // Is it faster to reset only the old nonzero indices and set only the new
    // ones?
...
public:
  ...
  inline Segment()
      : _totalActivations(1), _positiveActivations(1), _lastActiveIteration(0),
        _lastPosDutyCycle(0.0), _lastPosDutyCycleIteration(0),
        _seqSegFlag(false), _frequency(0), _synapses(), _nConnected(0) {}
  ...
  inline bool checkConnected(Real permConnected) const {
    ...
    if (nc != _nConnected) {
      std::cout << "\nConnected stats inconsistent. _nConnected=" << _nConnected
                << ", computed nc=" << nc << std::endl;
    }
   ...
   * *not* a cell index, but just the index of the synapse on that segment, i.e.
   * that index will change if synapses are deleted from this segment in synpase
...
public:
   ...
   * - removed:     srcCellIdx of the synapses that are removed
   */
  void decaySynapses(Real decay, std::vector<UInt> &removed, Real permConnected,
                     bool doDecay = true);
   ...
   */
  void freeNSynapses(UInt numToFree, std::vector<UInt> &inactiveSynapseIndices,
