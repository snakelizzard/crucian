...
public:
    ...
    /**
     * Deallocates memory used by this instance. Doesn't change the number of
     * rows or columns.
     */
    inline void clear()
    {
     ...
     * If the new size is smaller than the old one, some non-zeros are
     * discarded. If the new size is larger than the old one, the non-zeros are
     * unchanged.
     */
     ...
     * first
     * and one past the last row, and the first and one past the last column,
     * i.e. the box is: [row_begin, row_end) X [col_begin, col_end).
     *
     ...
     * last col
     *  of the box to look into
     * @retval [0 <= size_type <= box size] number of non-zeros found in the
     * box, where box size = (row_end - row_begin) * (col_end - col_begin)
     *
     * @b Complexity:
    ...
    inline const Row &getSparseRow(size_type row) const
    {
        { // Pre-conditions
            NTA_ASSERT(/*0 <= row &&*/ row < nRows())
                << "SparseBinaryMatrix::getSparseRow: Invalid row index: "
                << row << " - Should be < number of rows: " << nRows();
        } // End pre-conditions
    ...
    inline void appendSparseCol(InputIterator ind, InputIterator ind_end)
    {
        { // Pre-conditions
            NTA_ASSERT(static_cast<size_type>(ind_end - ind) <= nRows())
                ...
                << "Invalid vector size: "
                << static_cast<size_type>(ind_end - ind)
                                 ...
                                 InputIterator end)
    {
        { // Pre-conditions
            NTA_ASSERT(/*0 <= row &&*/ row < nRows())
                << "SparseBinaryMatrix::replaceSparseRow: Invalid row index: "
                << row << " - Should be < number of rows: " << nRows();
    ...
    */
                         ...
                         const Other &src)
    {
        ...
        for (size_type row = 0; row != static_cast<size_type>(src.nRows());
             ++row)
        {
            for (size_type col = 0; col != static_cast<size_type>(src.nCols());
                 ++col)
            {
        ...
        { // Pre-conditions
            ...
            */
            ...
            NTA_ASSERT(nrows * ncols <= static_cast<size_type>(end - begin))
                ...
                << " and: " << ncols << " when storage has size: "
                << static_cast<size_type>(end - begin);
        } // End pre-conditions
    ...
    inline bool equals(const SparseBinaryMatrix &o) const
    {
        ...
        for (size_type row = 0; row != nRows(); ++row)
        {
            if (o.nNonZerosOnRow(row) != nNonZerosOnRow(row))
                return false;
            if (!std::equal(ind_[row].begin(), ind_[row].end(),
                            o.ind_[row].begin()))
                return false;
        }
        return true;
                                ...
                                OutputIterator y, OutputIterator y_end) const
    {
        { // Pre-conditions
            NTA_ASSERT(static_cast<size_type>(x_end - x) >= nCols())
                ...
                << static_cast<size_type>(x_end - x)
                << " - Should >= number of colums: " << nCols();

            NTA_ASSERT(static_cast<size_type>(y_end - y) >= nRows())
                << "SparseBinaryMatrix::rightVecSumAtNZ: "
                << "Invalid output vector size: "
                << static_cast<size_type>(y_end - y)
