...
#define ITERATE_ON_BOX_COLS                                                    \
    ...
    /**
     * These common asserts also make the code more streamlined and more
     * readable.
     */
        ...
        for (size_type j = 0, prev = 0; ind_it != ind_end; ++ind_it, ++j)
        {
            ...
            if (j > 0)
            {
                NTA_ASSERT(prev < index)
                    ...
                    << " without duplicates, found: " << prev << " and "
                    << index;
            }
                                        ...
                                        const char *where) const
    {
        ...
        for (size_type j = 0, prev = 0; ind_it != ind_end;
             ++ind_it, ++nz_it, ++j)
        {
        ...
        ITERATE_ON_ALL_ROWS
        {
            ITERATE_ON_ROW
            {
                ...
                NTA_ASSERT(*ind < nCols())
                    << where << "Invalid col index: " << *ind
                    << " nCols= " << nCols();
            }

            assert_valid_sorted_index_range_(nCols(), ind_begin_(row),
                                             ind_end_(row), where);
        }
     ...
     *
     * @param ncols [0 <= size_type] the new number of columns to use to
    ...
    inline void reserve_(size_type new_nrow)
    {
        { // Pre-conditions
            NTA_ASSERT(0 <= new_nrow)
                << "SparseMatrix reserve_: Bad new number of rows: "
                << new_nrow;
        } // End pre-conditions
        ...
        if (new_nrow > nrows_max_ - 1)
        {
            ...
            std::fill(ind_new + nrows_, ind_new + nrows_max_,
                      (size_type *)nullptr);
            std::fill(nz_new + nrows_, nz_new + nrows_max_,
                      (value_type *)nullptr);
     ...
     * @b Requirements
     *   @li Columns must be sorted
     *   @li The caller must correctly count the number of zeros in the
     * selection
     */
    template <typename InputIterator, typename Random>
    inline void insertRandomNonZerosIntoColumns_(size_type row,
                                                 InputIterator col_begin,
                                                 InputIterator col_end,
                ...
                while (true)
                {
                    ...
                    else
                    {
                        ...
                        const bool insertNonzero =
                            (rng.getUInt32(numRemainingAvailable) <
...
public:
     ...
     */
    inline SparseMatrix()
        : nrows_(0), nrows_max_(0), ncols_(0), nnzr_(nullptr),
          ind_mem_(nullptr), nz_mem_(nullptr), ind_(nullptr), nz_(nullptr),
          indb_(nullptr), nzb_(nullptr), isZero_()
    {
        allocate_(0, 0);
    }
    ...
    inline SparseMatrix(size_type nrows, size_type ncols, InputIterator dense)
        : nrows_(0), nrows_max_(0), ncols_(0), nnzr_(0), ind_mem_(0),
          nz_mem_(0), ind_(0), nz_(0), indb_(0), nzb_(0), isZero_()
    {
        { // Pre-conditions
            NTA_CHECK(nrows >= 0)
                                   ...
                                   InputIterator take, InputIterator take_end)
    {
        ...
        for (size_type row = 0; take != take_end; ++take, ++row)
        {
            if (*take == 1)
            {
                ...
                std::copy(other.ind_begin_(row), other.ind_end_(row),
                          ind_[row]);
     ...
     *  @li None.
     */
    template <typename UI2, typename R2, typename I2, typename RP2,
              typename DTZ2>
    inline void copy(const SparseMatrix<UI2, R2, I2, RP2, DTZ2> &other)
    {
        ...
        ITERATE_ON_ALL_ROWS
        {
            ...
            ind_[row] = indp;
            nz_[row] = nzp;
            std::copy(other.row_nz_index_begin(row),
                      other.row_nz_index_end(row), indp);
            std::copy(other.row_nz_value_begin(row),
                      other.row_nz_value_end(row), nzp);
            indp += nnz;
            nzp += nnz;
        }
    }
    ...
    /**
     * Destructor.
     * Internal data structures are freed. Pointers and data members are set to
     * 0.
     *
     * @b Complexity:
     *  @li Worst case: O(nrows) if not compact
    ...
    inline size_type nNonZerosOnCol(size_type col) const
    {
        ...
        ITERATE_ON_ALL_ROWS if (col_(row, col) >= 0)++ nnzc;
    ...
    inline size_type nNonZeros() const
    {
        ...
        ITERATE_ON_ALL_ROWS nnz += nNonZerosOnRow(row);
    ...
    inline bool isColZero(size_type col) const
    {
        ...
        ITERATE_ON_ALL_ROWS if (col_(row, col) >= 0) return false;
    ...
    inline size_type nNonZeroRows() const
    {
        ...
        ITERATE_ON_ALL_ROWS if (nnzr_[row] > 0)++ count;
    ...
    /**
     * Returns the number of zero cols, i.e. the number of columns that don't
     * have any non-zero.
     *
    ...
    lastNonZeroOnCol(size_type col) const
    {
        ...
        for (int row = (int)nRows() - 1; row != -1; --row)
        {
            size_type *j = pos_(row, col);
            if (j != ind_end_(row) && *j == col)
                return std::make_pair((size_type)row,
                                      nz_[row][j - ind_begin_(row)]);
        }
    ...
    /**
     * Returns the number of rows between the last and the first non-zero of
     * each column, for all columns simultaneously.
     *
     * @param it [OutputIterator] iterator to the beginning of the range that
     * will receive the column bandwidths. It will iterate nCols() times.
     *
     * @b Complexity:
    ...
    /**
     * Returns the number of non-zeros for each box in a collection of boxes
     * that partitions the matrix. The indices of the rows and columns that
     * partition the matrix are passed in two ranges. The result is put into an
     * object of type Summary that must have a resize and a set methods.
     *
     ...
     * @param summary [Summary&] storage for the result, must have resize and
     * set methods
     *
     * @b Complexity:
     *  @li O(2 * (row_end - row_begin) * log(nnzr))
     ...
     *
     * @param it [OutputIterator] iterator to the beginning of the range that
     * will receive the indices of the non-zero rows. It will iterate at most
     * nRows() times.
     *
     * @b Complexity:
     *  @li O(nrows)
    ...
    {
        ITERATE_ON_ALL_ROWS if (!this->isRowZero(row)) *it++ = row;
    }
     ...
     *
     * @param it [OutputIterator] iterator to the beginning of the range that
     * will receive the indices of the zero rows. It will iterate at most
     * nRows() times.
     *
     * @b Complexity:
     *  @li O(nrows)
     ...
     *
     * @param it [OutputIterator] iterator to the beginning of the range that
     * will receive the indices of the non-zero columns. It will iterate at most
     * nCols() times.
     *
     * @b Complexity:
     *  @li O(ncols)
    ...
    {
        ITERATE_ON_ALL_COLS if (!this->isColZero(col)) *it++ = col;
    }
     ...
     *
     * @param it [OutputIterator] iterator to the beginning of the range that
     * will receive the indices of the zero columns. It will iterate at most
     * nCols() times.
     *
     * @b Complexity:
     *  @li O(ncols)
    ...
    /**
     * For a matrix that has as many rows as columns, returns the list of
     * indices that correspond to a zero row and a zero column simultaneously.
    ...
    inline bool sameNonZeroIndices(const SparseMatrix &B) const
    {
        ...
        ITERATE_ON_ALL_ROWS if (!sameRowNonZeroIndices(row, B)) return false;

        return true;
    }

    /**
     * Checks whether the indices of the non-zeros on row of this SM are a
     * subset of the indices of the non-zeros of the same row of the B sparse
     * matrix. In the case of a row of zeros in this SM, that means the set of
     * indices of non-zeros is empty, and the empty set is always included in
     * the set of non-zeros of B.
     */
    ...
    inline bool nonZeroIndicesIncluded(const SparseMatrix &B) const
    {
        ...
        ITERATE_ON_ALL_ROWS if (!nonZeroIndicesIncluded(row, B)) return false;
    ...
    inline void toDense(OutputIterator dense) const
    {
        ...
        ITERATE_ON_ALL_ROWS getRowToDense(row, dense + row * ncols);
    }
    ...
    inline void reshape(size_type new_nrows, size_type new_ncols)
    {
        { // Pre-conditions
            ...
            NTA_ASSERT((double)new_nrows * new_ncols ==
                       (double)nRows() * nCols())
                << "SparseMatrix reshape: "
...
/**
     ...
     *
     * @param src [Domain2D] the source domain to extract from this sparse
     * matrix
...
/**
     ...
     * the permutation, the first col should be the old col 3, the second the
     * old col 2...
     *
...
/**
     ...
     * @param f1 [unary_function] a unary function to apply to each element of
     * the column
     *
     * @b Exceptions:
     *  @li If col < 0 || col >= ncols
...
/**
     ...
     *
     * @param tr [SparseMatrix<size_type, value_type>] the transpose sparse
     * matrix
     *
     * @b Exceptions:
     *  @li Not enough memory (error)
...
/**
     ...
     * @param f1 [UnaryPredicate] the unary predicate to use for testing
     * elements
...
/**
     ...
     * To adjust indices, subtract begin_row and begin_col afterwards.
     *
     * @param begin_row [0 <= size_type < nrows] the start row of the box
     * @param end_row [0 <= size_type <= nrows] one past the last row of the box
     * @param begin_col [0 <= size_type < ncols] the start col of the box
...
/**
     ...
     * matrix.
     * The first element in each pair is the column index of the maximum for
     * each row, and the second element is the value of that maximum.
     *
...
/**
     ...
     * There are as many pairs in that array as there are rows in the sparse
     * matrix.
     * The first element in each pair is the column index of the minimum for
     * each row, and the second element is the value of that minimum.
     *
...
/**
     ...
     * exact can be set to true, resulting in two passes of normalization
...
/**
     ...
     *  col in [0,ncols)
     *
     * @param sums [OutputIterator] iterator to storage for the sum of each
     * column
     *
     * @b Exceptions:
     *  @li None.
...
/**
     ...
     * smaller matrices, corresponding to the blocks.
     *
...
/**
     ...
     *   y[row] =
     *    max(lb,
     *        prod(this[row,col] * x[col], for col in [0,ncols) s.t.
     * this[row,col]
     * != 0))
     *
     * @param x [InputIterator<value_type>] input vector (size = number of
                                ...
                                const SparseMatrix<I, F, I2, F2, ZeroTest> &x)
{
    if (io_control.sparse_io == AS_DENSE)
    {
        ...
        const_cast<SparseMatrix<I, F, I2, F2, ZeroTest> &>(x).toBinary(
            out_stream);
    return out_stream;
}
