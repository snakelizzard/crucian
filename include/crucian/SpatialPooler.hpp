...
#include <crucian/SparseBinaryMatrix.hpp>
...
public:
    ...
    @param numActiveColumnsPerInhArea An alternate way to control the sparsity
    of active columns. If numActivePerInhArea is specified then localAreaDensity
    must be less than 0, and vice versa. When numActivePerInhArea > 0, the
    inhibition logic will insure that at most 'numActivePerInhArea' columns
    remain ON within a local inhibition area (the size of which is set by the
    internally calculated inhibitionRadius). When using this method, as columns
    ...
    */
    std::vector<UInt> getColumnDimensions() const;
    ...
    */
    std::vector<UInt> getInputDimensions() const;
    ...
    */
    ...
    @returns real number of the permanence decrement amount for inactive
    synapses.
    */
    ...
    */
    ...
    @param synPermBelowStimulusInc real number of the permanence increment
    amount for columns that have not been recently active, must be larger than
    0.
    */
    ...
    */
    ...
    @param synPermConnected real number of the permanence amount that qualifies
    a synapse as being connected.
    */
    ...
    */
    ...
    @param minPctOverlapDutyCycles real number of the minimum tolerated
    overlaps.
    */
    ...
    */
    ...
    @param minOverlapDutyCycles real arry to store mininum overlap duty cycles
    for all columns. 'minOverlapDutyCycles' size must match the number of
    columns.
    */
    ...
    @param minOverlapDutyCycles real array of the minimum overlap duty cycles
    for all columns.
    */
    ...
    */
     ...
     */
    const std::vector<UInt> &getOverlaps() const;

    /**
    Returns the boosted overlap score for each column.
     */
    const std::vector<Real> &getBoostedOverlaps() const;
    ...
    /**
      ...
      what is the index of the flattened input vector that is to be the center
      of the column's potential pool. It distributes the columns over the inputs
      ...
      * If the topology is one dimensional, and the column index is 0, this
        method will return the input index 0. If the column index is 1, and
      there are 3 columns over 7 inputs, this method will return the input
      index 3.
    ...
    */
    ...
    /**
      ...
      * If the potentialRadius is greater than or equal to the entire input
        space, (global visibility), then this method returns an array filled
      ...
      * If the topology is two dimensional (not implemented), and the
      ...
      @param wrapAround  A boolean value indicating that boundaries should be
                         ignored.
    */
    std::vector<UInt> mapPotential_(UInt column, bool wrapAround);
    ...
    */
    ...
    /**
      ...
      @param connectedPct   A real value between 0 or 1 specifying the percent
      of the input bits that will start off in a connected state.
    */
    ...
    /**
       ...
       are always between 0 and 1 - and 'trimming' - enforcing sparsity by
       zeroing out all permanence values below '_synPermTrimThreshold'. It also
       maintains the consistency between 'self._permanences' (the matrix storing
       the permanence values), 'self._connectedSynapses', (the matrix storing
       the bits each column is connected to), and 'self._connectedCounts' (an
       array storing the number of input bits each column is connected to).
       Every method wishing to modify the permanence matrix should do so through
       this method.

        ----------------------------
        @param perm           An int vector of permanence values for a column.
       The array is "dense", i.e. it contains an entry for each input bit, even
       if the permanence value is 0.

        @param column          An int number identifying a column in the
       permanence, potential and connectivity matrices.

        @param  raisePerm      a boolean value indicating whether the permanence
       values should be raised until a minimum number are synapses are in a
       connected state. Should be set to 'false' when a direct assignment is
       required.
    */
    ...
    */
    ...
    /**
        Performs inhibition. This method calculates the necessary values needed
       to actually perform inhibition and then delegates the task of picking the
        active columns to helper functions.


        @param overlaps       an array containing the overlap score for each
       column. The overlap score for a column is defined as the number of
       synapses in a "connected state" (connected synapses) that are connected
       to input bits which are turned on.

        @param activeColumns an int array containing the indices of the active
       columns.
    */
    void inhibitColumns_(const std::vector<Real> &overlaps,
    ...
    */
    void inhibitColumnsGlobal_(const std::vector<Real> &overlaps, Real density,
                               std::vector<UInt> &activeColumns);
    ...
    /**
       ...
       an int array containing the indices of the active columns.
    */
    void inhibitColumnsLocal_(const std::vector<Real> &overlaps, Real density,
                              std::vector<UInt> &activeColumns);
    ...
    /**
       ...
       to input bits that are turned on, and decreased for synapses connected to
        ...
        ----------------------------
        @param inputVector    an int array of 0's and 1's that comprises the
       input to the spatial pooler. There exists an entry in the array for every
       input bit.
    ...
    /**
        ...
        activity level has been too low. Such columns are identified by having
       an overlap duty cycle that drops too much below those of their peers. The
    ...
    */
    ...
    /**
       ...
       dimensions. If the number of column dimensions does not match the number
       of input dimensions, we treat the missing, or phantom dimensions as
       'ones'.
    ...
    */
    ...
    /**
        Updates the minimum duty cycles in a global fashion. Sets the minimum
       duty cycles for the overlap and activation of all columns to be a percent
       of the maximum in the region, specified by minPctOverlapDutyCycle and
    ...
    */
    ...
    /**
        ...
        the updated duty cycle. Duty cycles are updated according to the
       following formula:
        @verbatim
       ...
       values that need to be updated.
        ...
        @param period         A int number indicating the period of the duty
       cycle
    */
    static void updateDutyCyclesHelper_(std::vector<Real> &dutyCycles,
    ...
    in a "connected state" (connected synapses) that are connected to input bits
    which are turned on.
    ...
    */
    ...
    /**
      ...
      boosted. Columns whose active duty cycle drops too much below that of
      their neighbors are boosted depending on how infrequently they have been
      active. Columns that has been active more than the target activation level
      have a boost factor below 1, meaning their overlap is suppressed

      The boostFactor depends on the activeDutyCycle via an exponential
      function:
    ...
    */
    ...
    Updates counter instance variables each round.
      ...
      @param learn          a boolean value indicating whether learning should
    be performed. Learning entails updating the  permanence values of the
    synapses, and hence modifying the 'state' of the model. setting learning to
    'off' might be useful for indicating separate training vs. testing sets.
     */
    void updateBookeepingVars_(bool learn);

    /**
    @returns boolean value indicating whether enough rounds have passed to
    warrant updates of duty cycles
    */
    bool isUpdateRound_();
...
protected:
    UInt numInputs_;
    UInt numColumns_;
    std::vector<UInt> columnDimensions_;
    std::vector<UInt> inputDimensions_;
    ...
    std::vector<Real> boostFactors_;
    std::vector<Real> overlapDutyCycles_;
    std::vector<Real> activeDutyCycles_;
    std::vector<Real> minOverlapDutyCycles_;
    std::vector<Real> minActiveDutyCycles_;
    ...
    std::vector<UInt> connectedCounts_;

    std::vector<UInt> overlaps_;
    std::vector<Real> overlapsPct_;
    std::vector<Real> boostedOverlaps_;
    std::vector<UInt> activeColumns_;
    std::vector<Real> tieBreaker_;
