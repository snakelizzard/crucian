...
#include <crucian/OutSynapse.hpp>
...
#define REPORT_ACTIVITY_STATISTICS 0
        ...
        if (_size == 0)
        {
            std::cout << "Reset width=" << sizeof(It) << " all zeroes"
                      << std::endl;
        }
        else
        {
        ...
        {             // if fewer than 6.25% are nonzero
...
public:
    ...
    //-----------------------------------------------------------------------
    /**
     * Can return a previously freed segment (segment size == 0) if called with
     * a segIdx which is in the "free" list of the cell.
     */
     ...
     *
     */
    bool inferPhase1(const std::vector<UInt> &activeColumns,
                     bool useStartCells);
    ...
    //----------------------------------------------------------------------
    /**
     * Insert a segmentUpdate data structure containing a list of proposed
     * changes to segment segIdx. If newSynapses is true, then newSynapseCount -
     * len(activeSynapses) synapses are added to activeSynapses. These synapses
     * are randomly chosen from the set of cells that have learnState = 1 at
     * timeStep.
     *
    ...
    //----------------------------------------------------------------------
    /**
     * Adds OutSynapses to the internal data structure that maintains
     * OutSynapses for each InSynapses. This enables us to propagation
     * activation forward, which is faster since activation is sparse.
     *
     ...
     *
     * elseif   the cell is still being predicted, and pooling is on then leave
     * it in the queue
     *
     ...
     * predictedState:  array of _nCells states representing predictions for
     * each cell
     *
     */
    ...
    //-----------------------------------------------------------------------
    /**
     * Returns list of indices of segments that are *not* empty in the free
     * list.
     */
