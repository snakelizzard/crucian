...
struct IOControl
{
    ...
    int convert_from_sparse; // convert from pos. of non-zero to dense 0/1
                             // vector
...
inline void binary_save(std::ostream &out_stream, It begin, It end)
{
    ...
    if (size > 0)
    {
        ...
        out_stream.write(ptr, static_cast<std::streamsize>(size) *
                                  sizeof(value_type));
    }
}
...
inline void binary_load(std::istream &in_stream, It begin, It end)
{
    ...
    if (size > 0)
    {
        char *ptr = const_cast<char *>(reinterpret_cast<const char *>(&*begin));
        in_stream.read(ptr,
                       static_cast<std::streamsize>(size) * sizeof(value_type));
    }
}
