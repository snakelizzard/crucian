...
#include <crucian/ArrayAlgo.hpp> // is_sorted
#include <crucian/InSynapse.hpp>
#include <crucian/StlIo.hpp> // binary_save
...
const UInt _dutyCycleTiers[] = {0,     100,   320,    1000,  3200,
                                10000, 32000, 100000, 320000};
...
public:
    //----------------------------------------------------------------------
    inline Segment()
        : _totalActivations(1), _positiveActivations(1),
          _lastActiveIteration(0), _lastPosDutyCycle(0.0),
          _lastPosDutyCycleIteration(0), _seqSegFlag(false), _frequency(0),
          _synapses(), _nConnected(0)
    {
    }

    //----------------------------------------------------------------------
    Segment(InSynapses _s, Real frequency, bool seqSegFlag, Real permConnected,
            UInt iteration);
    ...
    //-----------------------------------------------------------------------
    /**
     * Returns the permanence of the idx-th synapse on this Segment. That idx is
     * *not* a cell index, but just the index of the synapse on that segment,
     * i.e. that index will change if synapses are deleted from this segment in
     * synpase adaptation or global decay.
     */
    ...
    //-----------------------------------------------------------------------
    /**
     * Note that _seqSegFlag is set back to zero when the synapses are erased:
     * when a segment is released, it's empty _AND_ it's no long a sequence
     * segment. This simplifies further tests in the algorithm.
     */
    inline void clear()
    {
        _synapses.clear();
     ...
     * Parameters:
     * ==========
     * - srcCells:        a collection of source cell indices (the sources of
     * the synapses). Source cell indices are unique on a segment, and are kept
     * in increasing order.
     * - initStrength:    the initial strength to set for the new synapses
     */
    void addSynapses(const std::set<UInt> &srcCells, Real initStrength,
                     Real permConnected);
...
public:
    //-----------------------------------------------------------------------
    /**
     * Updates synapses permanences, possibly removing synapses from the segment
     * if their permanence drops to or below 0.
     *
     * Parameters:
     * ==========
     * - synapses:       a collection of source cell indices to update (will be
     * matched with source cell index of each synapse)
     * - delta:          the amount to add to the permanence value of the
     * updated synapses
     * - removed:        collection of synapses that have been removed because
     * their permanence dropped below 0 (srcCellIdx of synapses).
     *
     ...
     * inactiveSegmentIndices:    list of inactive synapses (index within
     * segment) activeSynapseIndices:      list of active synapses (src cell
     * indices) activeSegmentIndices:      list of active synapses (index within
     * segment) removed:                   srcCellIdx of the synapses that are
     *                            removed
     ...
     */
    bool isActive(const CState &activities, Real permConnected,
