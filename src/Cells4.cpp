...
// indices
//--------------------------------------------------------------------------------
static void printActiveColumns(std::ostream &out,
 ...
 * to segment s into our SegmentUpdate list. Return false if no update was
 * actually pushed (this can happen if we didn't find any new synapses).
 *
 ...
 *
 */
bool Cells4::computeUpdate(UInt cellIdx, UInt segIdx,
                           CStateIndexed &activeState, bool sequenceSegmentFlag,
                           bool newSynapsesFlag)
{
    {
        NTA_ASSERT(cellIdx < nCells());
    ...
    if (newSynapsesFlag)
    {
        ...
        if (nSynToAdd > 0)
        {
            chooseCellsToLearnFrom(cellIdx, segIdx, nSynToAdd, activeState,
                                   newSynapses);
        }
    }
    ...
    SegmentUpdate update(cellIdx, segIdx, sequenceSegmentFlag, _nLrnIterations,
                         newSynapses); // TODO: Add this for invariants check
...
void Cells4::inferBacktrack(const std::vector<UInt> &activeColumns)
{
    ...
    for (; startOffset < _prevInfPatterns.size(); startOffset++)
    {
        ...
        for (size_t offset = startOffset; offset < _prevInfPatterns.size();
             offset++)
        {
            // If we are about to set the active columns for the current time
            // step based on what we predicted, capture and save the total
            // confidence of predicting the current input
        ...
        else
        {
            ...
            {
                std::cout << "# Prediction confidence of current input after "
                             "starting "
                          << _prevInfPatterns.size() - 1 - startOffset
            ...
            memcpy(_cellConfidenceCandidate, _cellConfidenceT,
                   _nCells * sizeof(_cellConfidenceT[0]));
            memcpy(_colConfidenceCandidate, _colConfidenceT,
                   _nColumns * sizeof(_colConfidenceT[0]));
    ...
    else
    {
        ...
        if (candStartOffset != (Int)currentTimeStepsOffset)
        {
            ...
            memcpy(_cellConfidenceT, _cellConfidenceCandidate,
                   _nCells * sizeof(_cellConfidenceCandidate[0]));
            memcpy(_colConfidenceT, _colConfidenceCandidate,
                   _nColumns * sizeof(_colConfidenceCandidate[0]));
        }
    }
...
bool Cells4::learnBacktrackFrom(UInt startOffset, bool readOnly)
{
    ...
    for (UInt offset = startOffset; offset < numPrevPatterns; offset++)
    {
        ...
        // Break out immediately if we fell out of sequence or reached the
        // current time step
...
UInt Cells4::learnBacktrack()
{
    ...
    if (numPrevPatterns <= 0)
    {
        ...
        {
            std::cout
                << "lrnBacktrack: No available history to backtrack from\n";
        }
        return false;
    }
    ...
    {
        std::cout
            << "Discovered path to current input by using start cells from "
...
UInt Cells4::getCellForNewSegment(UInt colIdx)
{
    ...
    for (UInt i = minIdx; i <= maxIdx; i++)
    {
        ...
        for (UInt segIdx = 0; segIdx < _cells[i].size(); segIdx++)
        {
            ...
            {
                Real dc =
                    _cells[i][segIdx].dutyCycle(_nLrnIterations, false, false);
    ...
    _cells[candidateCellIdx][candidateSegmentIdx].getSrcCellIndices(
        synsToRemove);
...
bool Cells4::learnPhase1(const std::vector<UInt> &activeColumns, bool readOnly)
{
    ...
    for (auto &activeColumn : activeColumns)
    {
        ...
        {
            //----------------------------------------------------------------------
            // If no predicted cell, pick the closest matching one to reinforce,
            // or if none exists, create a new segment on a cell in that column
            ...
            if (!readOnly)
            {
                    ...
                    _cells[cellIdx][segIdx].isSequenceSegment())
                {
                    ...
                    _learnActiveStateT.set(cellIdx);
                    bool newUpdate = computeUpdate(
                        cellIdx, segIdx, _learnActiveStateT1, true, true);
                    _cells[cellIdx][segIdx]._totalActivations++;

                    if (newUpdate)
                    {
                        // This will update the permanences,
                        // posActivationsCount, and the lastActiveIteration
                        // (age).
                ...
                else
                {
                    ...
                    bool newUpdate = computeUpdate(
                        newCellIdx, (UInt)-1, _learnActiveStateT1, true, true);

                    // This will update the permanences,  posActivationsCount,
                    // and the lastActiveIteration (age).
                    if (newUpdate)
                    {
                        const SegmentUpdate &update = _segmentUpdates.back();
                        adaptSegment(update);
...
void Cells4::learnPhase2(bool readOnly)
{
    ...
    for (UInt colIdx = 0; colIdx != _nColumns; ++colIdx)
    {
        ...
        std::pair<UInt, UInt> p;
        p = getBestMatchingCellT(colIdx, _learnActiveStateT,
                                 _activationThreshold);
        UInt cellIdx = p.first, segIdx = p.second;
        if (segIdx != (UInt)-1)
        {
            // Turn on the predicted state for the best matching cell and queue
            // the pertinent segment up for an update, which will get processed
            // if the cell receives bottom up in the future.
            _learnPredictedStateT.set(cellIdx);
            if (!readOnly)
            {
                if (_verbosity >= 4)
                {
                    std::cout << "learnPhase2, learning on col=" << colIdx
                              << ", cellIdx=" << cellIdx
                              << ", seg ID: " << segIdx << ", segment: ";
...
void Cells4::updateLearningState(const std::vector<UInt> &activeColumns)
{
    ...
    if (_verbosity >= 3)
    {
        std::cout << "pamCounter = " << _pamCounter
                  << ", learnedSeqLength = " << _learnedSeqLength << "\n";
    }
        ...
        ((_maxSeqLength != 0) && (_learnedSeqLength >= _maxSeqLength)))
    {
        if (_verbosity >= 3)
        {
            ...
            else if (_pamCounter == 0)
                std::cout << "(PAM counter expired)\n";
            else
                std::cout << "(reached maxSeqLength)\n";
        }

        // Update average learned sequence length - this is a diagnostic
        // statistic
        UInt seqLength = (_pamCounter == 0 ? _learnedSeqLength - _pamLength
                                           : _learnedSeqLength);
        if (_verbosity >= 3)
...
void Cells4::updateInferenceState(const std::vector<UInt> &activeColumns)
{
    ...
    _infPredictedStateT1 = _infPredictedStateT;
    memcpy(_cellConfidenceT1, _cellConfidenceT,
           _nCells * sizeof(_cellConfidenceT[0]));

    // Copy over previous column confidences
    memcpy(_colConfidenceT1, _colConfidenceT,
           _nColumns * sizeof(_colConfidenceT[0]));
    ...
    if (!inSequence)
    {
        ...
        {
            std::cout
                << "Too much unpredicted input, re-tracing back to try and"
                         ...
                         bool useStartCells)
{
    ...
    // else, for each column turn on any predicted cells. If there are none,
...
bool Cells4::inferPhase2()
{
            ...
            if (_inferActivity.get(cellIdx) >= _activationThreshold)
            {
                ...
                for (UInt j = 0; j != _cells[cellIdx].size(); ++j)
                {
                    ...
                    if (_checkSynapseConsistency)
                    {
                        ...
                        NTA_CHECK(numActiveSyns ==
                                  _inferActivity.get(cellIdx, j));
                    }
                    ...
                    if (_inferActivity.get(cellIdx, j) >= _activationThreshold)
                    {
                        ...
                        // Incorporate the confidence into the owner cell and
                        // column Use segment::getLastPosDutyCycle() here
                        Real dc = _cells[cellIdx][j].dutyCycle(_nLrnIterations,
                                                               false, false);
                        _cellConfidenceT[cellIdx] += dc;
                        _colConfidenceT[c] += dc;

                        // If we reach threshold on the connected synapses,
                        // predict it
    ...
    {
        for (UInt c = 0; c < _nColumns; c++)
            _colConfidenceT[c] /= sumColConfidence;
...
void Cells4::_updateAvgLearnedSeqLength(UInt prevSeqLength)
{
    ...
    _avgLearnedSeqLength =
        (1.0 - alpha) * _avgLearnedSeqLength + alpha * (Real)prevSeqLength;
    if (_verbosity >= 5)
    {
                                   ...
                                   const CState &predictedState)
{
    ...
    for (UInt i = 0; i != _segmentUpdates.size(); ++i)
    {
        ...
        else
        {
            ...
            // If we received bottom up input, then adapt this segment and
            // schedule update for removal
            if (input[colIdx] == 1)
            {
            ...
            {
                // We didn't receive bottom up input. If we are not (pooling and
                // still predicting) then delete this update
    ...
    for (UInt i = 0; i != _segmentUpdates.size(); ++i)
    {
        ...
        if (_verbosity >= 4)
        {
            std::cout << "\nIn cleanUpdatesList. _nLrnIterations: "
                      << _nLrnIterations << " checking segment: ";
            update.print(std::cout, true, _nCellsPerCol);
            std::cout << std::endl;
        }
    ...
    if (_globalDecay != 0 && (_maxAge > 0) && (_nLrnIterations % _maxAge == 0))
    {
        for (UInt cellIdx = 0; cellIdx != _nCells; ++cellIdx)
        {
            for (UInt segIdx = 0; segIdx != _cells[cellIdx].size(); ++segIdx)
            {
                ...
                if (age > _maxAge)
                {
                    ...
                    seg.decaySynapses2(_globalDecay, removedSynapses,
                                       _permConnected);
        ...
        if (_verbosity >= 3)
        {
            std::cout << "CPP Global decay decremented " << nSegmentsDecayed
                      << " segments and removed " << nSynapsesRemoved
                      << " synapses\n";
            std::cout << "_nLrnIterations = " << _nLrnIterations
                      << ", _maxAge = " << _maxAge
                      << ", globalDecay = " << _globalDecay << "\n";
        }
    } // (_globalDecay)
}
...
void Cells4::_generateListsOfSynapsesToAdjustForAdaptSegment(
    ...
    std::vector<UInt> &activeSrcCellIdxs, std::vector<UInt> &activeSynapseIdxs)
{
    // Purge residual data
...
void Cells4::adaptSegment(const SegmentUpdate &update)
{
    ...
    { // modify an existing segment
        ...
        // a search through pending updates each time a segment has been
        // deleted.
        ...
        if (_verbosity >= 4)
        {
            ...
            std::cout << "Reinforcing segment " << segIdx << " for cell[" << col
                      << "," << cell << "]\n     before: ";
        ...
        // increment/decrement inline here? Also, why can't we grow potential
        // new synapses instead of creating a list of all synapses and then
        // erasing them from this list? Seems like a lot of extra memory
        // thrashing. Also, why don't we just send a list of synapse indices
        // into updateSynapses? It would be cleaner and avoid the need to keep
        // the synapses sorted.

        // Accumulate lists of synapses to decrement, increment, add, and remove

        // Union of source cell indexes corresponding to existing active
        // synapses in the given segment plus new synapses to be added to the
        // segment
        std::set<UInt> synapsesSet(update.begin(), update.end());

        // NOTE: the following variables were declared static as a performance
        // optimization in the legacy code in order to reduce memory
        // allocations. The side effect is that this code is not thread-safe. At
        // the time, processes were the mechanism for parllelizing execution of
        // the algorithms.

        // Tracks source cell indexes corresponding to synapses in
        // the given segment that have been removed during execution of this
        // method
        static std::vector<UInt> removed;
        // Source cell indexes corresponding to synapses in the given segment
        // whose permances are to be decremented/incremented; ordered by index
        // of those synapses within the segment
        static std::vector<UInt> synToDec, synToInc;
        // Indexes of synapses within the current segment corresponding to
        // synapses that are inactive/active in ascending order; these variables
        // correlate with synToDec and synToInc.
        static std::vector<UInt> inactiveSegmentIndices, activeSegmentIndices;

        // Purge residual data from static variable; the others will be purged
        // by _generateListsOfSynapsesToAdjustForAdaptSegment
        removed.clear();
        ...
        if (!removed.empty())
        {
            ...
            _generateListsOfSynapsesToAdjustForAdaptSegment(
                segment, synapsesSet, synToDec, inactiveSegmentIndices,
        ...
        // If we have fixed resources, get rid of some old synapses, if
        // necessary
        if ((_maxSynapsesPerSegment > 0) &&
            (synapsesSet.size() + segment.size() >
             (UInt)_maxSynapsesPerSegment))
        {
            // TODO: What's preventing numToFree from exceeding segment.size()?
            // If you know, add a comment explaining it. If it exceeds, it will
            // cause memory corruption in Segment::freeNSynapses.
            UInt numToFree =
                synapsesSet.size() + segment.size() - _maxSynapsesPerSegment;
            segment.freeNSynapses(numToFree, synToDec, inactiveSegmentIndices,
                                  synToInc, activeSegmentIndices, removed,
                                  _verbosity, _nCellsPerCol, _permMax);
        }
    ...
    { // create new segment
        ...
        UInt si = _cells[cellIdx].getFreeSegment(
        ...
        // related counts
        _cells[cellIdx][si]._lastActiveIteration = _nLrnIterations;
        _cells[cellIdx][si]._positiveActivations = 1;
        _cells[cellIdx][si]._totalActivations = 1;
        ...
        if (_verbosity >= 3)
        {
            ...
            _cells[cellIdx][si].print(std::cout, _nCellsPerCol);
            std::cout << std::endl;
        }

        addOutSynapses(cellIdx, si, update.begin(), update.end());
    }
...
void Cells4::trimOldSegments(UInt maxAge)
{
    ...
    std::cout << "In trimOldSegments. Removed " << nSegsRemoved
              << " segments\n";
    NTA_CHECK(invariants());
}
...
void Cells4::save(std::ostream &outStream) const
{
    ...
    outStream << version() << " " << _rng << " "
              ...
              << _segUpdateValidDuration << " " << _initSegFreq << " "
              << _permInitial << " " << _permConnected << " " << _permMax << " "
              << _permDec << " " << _permInc << " " << _globalDecay << " "
              << _doPooling << " " << _maxInfBacktrack << " "
              << _maxLrnBacktrack << " " << _pamLength << " " << _maxAge << " "
              << _avgInputDensity << " " << _pamCounter << " " << _maxSeqLength
              << " " << _avgLearnedSeqLength << " " << _nLrnIterations << " "
              << _maxSegmentsPerCell << " " << _maxSynapsesPerSegment << " "
              << std::endl;

    // Additions in version 1.
    outStream << _learnedSeqLength << " " << _verbosity << " "
              << _checkSynapseConsistency << " " << _resetCalled << std::endl;
    outStream << _learnActiveStateT << " " << _learnActiveStateT1 << " "
              << _learnPredictedStateT << " " << _learnPredictedStateT1
              << std::endl;
...
void Cells4::saveToFile(const std::string &filePath) const
{
    ...
    // Request std::ios_base::failure exception upon logical or physical i/o
    // error
...
void Cells4::load(std::istream &inStream)
{
    ...
    inStream >> _activationThreshold >> _minThreshold >> _newSynapseCount >>
        nIterations >> _segUpdateValidDuration >> _initSegFreq >>
        _permInitial >> _permConnected >> _permMax >> _permDec >> _permInc >>
        _globalDecay >> _doPooling;
    ...
    inStream >> _maxInfBacktrack >> _maxLrnBacktrack >> _pamLength >> _maxAge >>
        _avgInputDensity >> _pamCounter >> _maxSeqLength >>
        _avgLearnedSeqLength >> _nLrnIterations >> _maxSegmentsPerCell >>
        _maxSynapsesPerSegment;

    if (v >= 1)
    {
        inStream >> _learnedSeqLength >> _verbosity >>
            _checkSynapseConsistency >> _resetCalled;
...
bool Cells4::invariants(bool verbose) const
{
    ...
    if (!consistent)
    {
        std::cout << "synapses inconsistent forward_map size="
                  << forward_map.size() << " back_map size=" << back_map.size()
                  << std::endl;
        // std::cout << "\nBack/forward maps: "
    ...
    const std::vector<std::pair<UInt, UInt>> &extSynapses)
{
    ...
    for (UInt i = 0; i != extSynapses.size(); ++i)
        synapses[i] =
            extSynapses[i].first * _nCellsPerCol + extSynapses[i].second;

    SegmentUpdate update(cellIdx, (UInt)-1, sequenceSegmentFlag,
                         _nLrnIterations,
                         synapses); // TODO: Add this for invariants check

    _segmentUpdates.push_back(update);
}

//--------------------------------------------------------------------------------
void Cells4::updateSegment(
    UInt colIdx, UInt cellIdxInCol, UInt segIdx,
    const std::vector<std::pair<UInt, UInt>> &extSynapses)
{
    NTA_ASSERT(colIdx < nColumns());
    NTA_ASSERT(cellIdxInCol < nCellsPerCol());
...
void Cells4::initialize(UInt nColumns, UInt nCellsPerCol,
                        UInt activationThreshold, UInt minThreshold,
...
std::vector<UInt> Cells4::getNonEmptySegList(UInt colIdx, UInt cellIdxInCol)
{
    NTA_ASSERT(colIdx < nColumns());
    NTA_ASSERT(cellIdxInCol < nCellsPerCol());
 ...
 */
std::pair<UInt, UInt> Cells4::getBestMatchingCellT(UInt colIdx,
                                                   const CState &state,
                                                   UInt minThreshold)
{
    {
    ...
    int start = static_cast<int>(colIdx * _nCellsPerCol),
        end = static_cast<int>(start + _nCellsPerCol);
    ...
    { // reverse segment order to match Python logic
        UInt i = UInt(ii);
        ...
        if (_checkSynapseConsistency)
        {
            for (UInt j = 0; j != _cells[i].size(); ++j)
            {
                NTA_CHECK(segment(i, j).computeActivity(state, _permConnected,
                                                        false) ==
                          _learnActivity.get(i, j));
            }
        }

        if (_learnActivity.get(i) > best_activity)
        { // if this cell may have a worthy segment
            ...
            for (UInt j = 0; j != _cells[i].size(); ++j)
            {
                ...
                {
                    std::cout
                        << "getBestMatchingCell, learning on col=" << colIdx
                        << ", segment: ";
                    _cells[i][j].print(std::cout, _nCellsPerCol);
                    std::cout << "\n";
                    std::cout << "activity = " << activity
                              << ", maxSegActivity = " << best_activity << "\n";
                }
 ...
 * Find weakly activated cell in column.
 */
std::pair<UInt, UInt> Cells4::getBestMatchingCellT1(UInt colIdx,
                                                    const CState &state,
                                                    UInt minThreshold)
{
    {
    ...
    for (UInt i = start; i != end; ++i)
    {
        ...
        for (UInt j = 0; j != _cells[i].size(); ++j)
        {
            ...
            activity =
                segment(i, j).computeActivity(state, _permConnected, false);
...
void Cells4::chooseCellsToLearnFrom(UInt cellIdx, UInt segIdx, UInt nSynToAdd,
                                    CStateIndexed &state,
                                    std::vector<UInt> &srcCells)
{
    ...
    if (segIdx != (UInt)-1)
    {
        ...
        if (vecPruned.size() <
            vecCellBuffer.size()) // ensure there is enough room for the results
            vecPruned.resize(vecCellBuffer.size());
        std::vector<UInt>::iterator iterPruned;
        iterPruned = std::set_difference(
            vecCellBuffer.begin(), vecCellBuffer.end(), vecAlreadyHave.begin(),
            vecAlreadyHave.end(), vecPruned.begin());
    ...
    if (nbrCells <= nSynToAdd)
    {
        ...
        for (iterCellBuffer = vecPruned.begin();
             iterCellBuffer != vecPruned.end(); ++iterCellBuffer)
        {
    ...
    else
    {
        ...
        _rng.sample(&vecPruned.front(), vecPruned.size(), &srcCells[start],
                    nSynToAdd);
...
std::pair<UInt, UInt> Cells4::trimSegments(Real minPermanence, UInt minNumSyns)
{
    ...
    for (UInt cellIdx = 0; cellIdx != _nCells; ++cellIdx)
    {
        for (UInt segIdx = 0; segIdx != _cells[cellIdx].size(); ++segIdx)
        {
            ...
            seg.decaySynapses(minPermanence, removedSynapses, minPermanence,
                              false);
...
void Cells4::computeForwardPropagation(CStateIndexed &state)
{
    ...
    std::vector<UInt>::iterator iterCellBuffer;
    for (iterCellBuffer = vecCellBuffer.begin();
         iterCellBuffer != vecCellBuffer.end(); ++iterCellBuffer)
    {
