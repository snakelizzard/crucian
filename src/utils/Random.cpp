...
double Random::getReal64() {
  ...
  Real64 dvalue = static_cast<Real64> (value); // No loss because we only need the 48 mantissa bits.
...
#endif
  state_[fptr_] =
      static_cast<UInt32> ((static_cast<UInt64> (state_[fptr_]) + static_cast<UInt64> (state_[rptr_])) % Random::MAX32);
  i = state_[fptr_];
...
RandomImpl::RandomImpl(UInt64 seed) {
  ...
  for (long i = 1; i < stateSize_; i++) {
    ...
    state_[i] = static_cast<UInt32> ((static_cast<UInt64> (test) + (test < 0 ? 2147483647 : 0)) % Random::MAX32);
  }
  fptr_ = sep_;
  rptr_ = 0;
#ifdef RANDOM_SUPERDEBUG
...
std::istream &operator>>(std::istream &inStream, RandomImpl &r) {
  ...
  UInt32 ss = 0;
  inStream >> ss;
  NTA_CHECK(ss == static_cast<UInt32> (RandomImpl::stateSize_)) << " ss = " << ss;
