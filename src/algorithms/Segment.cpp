...
bool Segment::operator==(const Segment &other) const {
  if (_totalActivations != other._totalActivations ||
      ...
      _seqSegFlag != other._seqSegFlag || nearlyEqual(_frequency, other._frequency) ||
...
Real Segment::dutyCycle(UInt iteration, bool active, bool readOnly) {
  ...
  // Update duty cycle
  dutyCycle = static_cast<Real> (pow(static_cast<Real64> (1.0f - alpha), static_cast<Real64> (age))) * _lastPosDutyCycle;
  if (active)
    dutyCycle += alpha;
...
void Segment::decaySynapses(Real decay, std::vector<UInt> &removed,
  ...
  for (UInt i = 0; i != _synapses.size(); ++i) {
    ...
    UInt wasConnected = static_cast<UInt> (_synapses[i].permanence() >= permConnected);
    ...
    UInt isConnected = static_cast<UInt>(_synapses[i].permanence() >= permConnected);
